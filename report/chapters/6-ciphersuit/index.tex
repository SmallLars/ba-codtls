...
eher eine Größe von ca 1200 - 1500 Byte zu berücksichtigen, die nur für die \acr{psr} benötigt wird. Um diese Probleme zu lösen bzw. die benötigte
...
Durch die Verwendung von \acr{ecc} bekommt das Ciphersuit nun seinen Namen:
\begin{itemize}
...
Schlüsselblocks innerhalb des Handshakes. Dieser wird gemäß RFC 5246 \cite{rfc5246} Kapitel 6.3 durch die \acr{psr} erzeugt und muss somit
...
Um auf \acr{hmac} und \acr{sha}2 verzichten zu können wird nun noch die \acr{psr} für dieses Ciphersuit definiert.
...
\end{description}
Grundlage zur Berechnung soll der \acr{cbc}-\acr{mac} sein, wobei der \acr{psk} direkt als Schlüssel genutzt wird.
Nach Vorbild von RFC 5246 \cite{rfc5246} wird die \acr{psr} nun gemäß Abbildung \ref{fig:def_psr} definiert, wobei
...
Um den Hash für die Berechnung der Finished-Nachricht zu ermitteln, wird nun ebenfalls der definierte \acr{cbc}-\acr{mac} benutzt.
