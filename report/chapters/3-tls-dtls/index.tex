...
zu kennzeichnen und voneinander abzugrenzen existiert das "`Record Layer Protkoll"' dessen Header in Abbildung \ref{fig:recordlayer}
...
Wenn es während des Handshakes oder der Übertragung von Anwendungsdaten zu fehlern kommt, werden diese mit Hilfe des Altert Protokolls übertragen.
Der Header (siehe Abbildung \ref{fig:alertlayer}) enthält neben dem AlertLevel, welches \text it{warning} (1) oder \textit{fatal} (2) sein kann,
...
\textit{warning} zur Information der Gegenseite über mögliche Probleme gedacht. Das Altert-Protokoll unterscheidet sich bei \acr{tls} und \acr{dtls}
nicht voneinander, da eine zuverlässige Übertragung nicht notwendig ist. Sollte aufgrund eines verloren gegangenen Altert-Pakets eine Anfrage
...
\begin{figure}[ht]
  ...
  \end{lstlisting}
  \caption{Header des Altert Protokolls von TLS / DTLS}
