...
aufgeführten \glospl{ciphersuite} in Frage. Diese nutzen für die Verschlüsselung den \acr{ccm}-Modus, der durch die Hardware einfach und effizient realisiert
werden kann. Während in Kapitel 3 des RFC's einige \glospl{ciphersuite} definiert sind, die zur Berechnung des Schlüssels das rechenaufwendige \glos{rsa} benutzen,
sind in Kapitel 4 des RFC's einige \glospl{ciphersuite} für \acr{aes}-128 definiert, die einen \acr{psk} verwenden und von RFC 4279 \cite{rfc4279} abgleitet wurden.
Der einzige Unterschied besteht hier darin, dass für die Berechnung des MAC in RFC 4279, der \acr{sha} verwendet wird, der hier nicht genutzt werden soll. Es kommen
somit die vier folgenden \glospl{ciphersuite} aus Kapitel 4 des RFC's in Frage:
...
berechnen. Damit kann Dieser die übertragenen Daten entschlüsseln oder manipulieren, unabhängig davon, ob er während des \glospl{handshake} gelauscht hat.
...
entschlüsseln. Problematisch bleibt nur ein \glos{mitma} während des \glospl{handshake}. Verhindert ein Angreifer die direkte
...
Problematisch ist jedoch, dass ein Diffie-Hellman-Schlüsselaustausch sehr rechenintensiv ist. Ein Versuch innerhalb des Bachelorprojekts \glos{gobi} hat
...
In Anlehnung an das \glos{ciphersuite} 4 soll nun zunächst die Effizienz des öffentlichen Schlüsselaustauschs verbessert werden. Um dieses zu realisieren,
...
\acr{ecc} und \acr{aes}-\acr{ccm} wurden zwar im Internet-Entwurf "`AES-CCM ECC Cipher Suites for TLS"' \cite{draftaesecc} schon kombiniert, jedoch
...
Alle weiteren Parameter für einen Diffie-Hellman-Schlüsselaustausch, unter Verwendung von \acr{ecc}, sind Teil der Aushandlung im \glos{handshake},
...
\begin{figure}[ht]
  ...
  \label{fig:aes_nonce}
\end{figure}
Während die Epoche und die Sequenznummer im \acr{dtls}-Header enthalten sind, und in der selben Ordnung (Network Byte Order == Most Significant
...
Keyblocks innerhalb des \glospl{handshake}. Dieser wird gemäß RFC 5246 \cite{rfc5246} Kapitel 6.3 durch die \acr{prf} erzeugt und muss somit
...
Für die Umsetzung sind zwei Parameter notwendig. Zum einen muss die Länge des \acr{mac} (M) definiert werden, welche in diesem Fall schon durch den Wert 8 festgelegt
...
Nach Vorbild von RFC 5246 \cite{rfc5246} wird die \acr{prf} nun gemäß Abbildung \ref{fig:def_psr} definiert, wobei + die Konkatenation 2er Zeichenketten bedeutet.
\begin{figure}[ht]
  \centering
  \begin{lstlisting}[language=c]
