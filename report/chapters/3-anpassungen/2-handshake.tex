...
Während in einem gewöhnlichen \acr{dtls}-\glos{handshake} jedes einzelne \acr{dtls}-\glos{handshake}-Paket sowohl den \acr{dtls}-Record-Header als auch den
\acr{dtls}-\glos{handshake}-Header enthält, ist dies hier nicht mehr notwendig, da mehrere \acr{dtls}-\glos{handshake}-Pakete innerhalb eines \acr{coap}-Pakets
enthalten sind. Der \acr{dtls}-Record-Header ist einmalig vor jedem \acr{coap}- oder Alert-Paket enthalten. Um die \acr{dtls}-\glos{handshake}-Pakete
...
enthalten sind. Neben \glos{handshake}- und ChangeCipherSpec-Paketen sind dort zusätzlich Alert-Pakete möglich. Während in den ersten sechs Bits, der in
...
\glos{handshake} ausgetauschten Pakete nun zusätzlich mit den neuen Sicherheitsparametern verschlüsselt, wobei das das Finished-Paket durch das
