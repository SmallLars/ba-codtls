...
Für die Realisierung des Handshakes über \acr{coap} dient die Ressource "`/dtls"'. Dieser \acr{uri} wurde bewusst kurz gehalten, um sowohl Daten
als auch Energie zu sparen, da er im Klartext in die \acr{coap}-Anfrage eingefügt wird. \acr{dtls}-Sessions, die während eines Handshakes erzeugt
...
da diese nicht zwingend ist, und die Session-ID im ServerHello-Paket enthalten ist, womit sich der neue \acr{uri} berechnen lässt. Die Verwendung
...
Während in einem gewöhnlichen \acr{dtls}-Handshake jedes einzelne \acr{dtls}-Handshake-Paket sowohl den \acr{dtls}-Record-Header als auch den
\acr{dtls}-Handshake-Header enthält, ist dies hier nicht mehr notwendig, da mehrere \acr{dtls}-Handshake-Pakete innerhalb eines \acr{coap}-Pakets
enthalten sind. Der \acr{dtls}-Record-Header ist einmalig vor jedem \acr{coap}- oder Alert-Paket enthalten. Um die \acr{dtls}-Handshake-Pakete
innerhalb eines \acr{coap}-Pakets voneinander abzugrenzen, dient nun der \acr{dtls}Conten-Header (Abbildung {ref{fig:com\_content\_header}).
...
Bytes der Länge, wobei der Wert 0 die Länge 0 direkt definiert. Neben den in \acr{dtls} definierten Handshake-Typen werden die folgenden
...
Je nach \glos{ciphersuite} wird dadurch die in \acr{tls} definierte Länge des Finished-Pakets von 12 vergrößert, da unter Umständen Zusatzinformationen
