...
Der RecordType (T) kann mit zwei Bit folgende 4 Zustände annehmen: \textit{8-Bit-Feld} (0), \textit{Alert} (1), \textit{\glos{handshake}} (2) und
\textit{Anwendungsdaten} (3). Trotz Realisierung des \glospl{handshake} über \acr{coap} ist diese Unterteilung notwendig, damit auch der DTLS-Layer über die
...
nach dem ersten \glos{handshake} in Epoche 1 fortgeführt wird, sind dies die am Häufigsten verwendeten Werte. Jeder weitere \glos{handshake} erhöht die Epoche um 1, so dass auch
